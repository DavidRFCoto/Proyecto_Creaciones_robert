...
class MedidaController extends Controller
{
    ...
    {
        // El Request ya validó que existan hombro, cintura y persona_id
        $persona = Persona::find($request->persona_id);
        if (!$persona) return response()->json(['error' => 'Persona no encontrada'], 404);

        $sexo = strtolower($persona->sexo);
        
        // 1. CÁLCULO DE TALLAS (Tus reglas de negocio)
        $resCamisa   = $this->calcularTallaCamisa($request->hombro);
        $resInferior = ['talla' => 'N/A', 'sug' => ''];
        $tipoPrenda  = "N/A";

        if ($sexo === 'masculino') {
            if ($request->cintura <= 64) {
                $resInferior = $this->calcularTallaShort($request->cintura);
                $tipoPrenda = "Short";
            } else {
                $resInferior = $this->calcularTallaPantalon($request->cintura);
                $tipoPrenda = "Pantalón";
            }
        } else if ($sexo === 'femenino') {
            $resInferior = $this->calcularTallaFalda($request->cadera);
            $tipoPrenda = "Falda";
        }
    ...
    // --- ANEXO A: CAMISAS (Tus Tablas) ---
    private function calcularTallaCamisa($h) {
        $tab = [
    ...
    // --- ANEXO B: PANTALONES ---
    private function calcularTallaPantalon($c) {
        $tab = [
    ...
    // --- ANEXO C: SHORTS ---
    private function calcularTallaShort($c) {
        $tab = [
    ...
    // --- ANEXO D: FALDA ---
    private function calcularTallaFalda($cad) {
        $tab = [
